    ...
    <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-slate-500 text-sm">Perbarui detail pejabat struktural: {{ $pegawai->nama }}</p>
            </div>
        </div>
    </div>

    {{-- Form --}}
    <form action="{{ route('admin.pegawai.update', $pegawai->id) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        @csrf
        @method('PUT')
        ...
        <div class="p-8 space-y-8">
            ...
            <div class="flex flex-col items-center justify-center space-y-4">
                <div class="relative group">
                    <div id="imagePreview" class="w-32 h-32 rounded-full border-4 border-slate-100 shadow-md overflow-hidden bg-slate-50 flex items-center justify-center text-slate-300">
                        @if($pegawai->foto)
                            <img src="{{ $pegawai->foto }}" class="w-full h-full object-cover">
                        @else
                ...
                <p class="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Foto Pegawai (Biarkan kosong jika tidak ingin mengubah)</p>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <select name="level" required
                        ...
                        <option value="kalapas" {{ $pegawai->level === 'kalapas' ? 'selected' : '' }}>Kalapas</option>
                        <option value="eselon_4" {{ $pegawai->level === 'eselon_4' ? 'selected' : '' }}>Pejabat Struktural Eselon IV</option>
                        <option value="eselon_5" {{ $pegawai->level === 'eselon_5' ? 'selected' : '' }}>Pejabat Struktural Eselon V</option>
                    </select>
                ...
                <div class="space-y-2">
                    ...
                    <input type="number" name="order_index" value="{{ old('order_index', $pegawai->order_index) }}" required
                           class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none">
            ...
            <div class="space-y-2">
                ...
                <textarea name="quotes" rows="3"
                          class="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all outline-none"
                          placeholder="Melayani dengan Hati, Berintegritas, dan Profesional...">{{ old('quotes', $pegawai->quotes) }}</textarea>
