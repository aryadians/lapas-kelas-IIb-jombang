...
<body class="bg-gray-100" x-data="{ sidebarOpen: false }">
    ...
    <div class="flex h-screen overflow-hidden">
        ...
        <aside :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full'" 
            ...
            <nav class="flex-1 px-4 py-6 space-y-4 overflow-y-auto sidebar-scroll">
                @php $userRole = Auth::user()->role ?? 'user'; @endphp

                {{-- DASHBOARD & STATISTIK --}}
                <div class="space-y-1">
                    <p class="px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3">Dashboard & Statistik</p>
                ...
                <div class="pt-3 border-t border-slate-800 space-y-1">
                    <p class="px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 mt-3">Layanan Kunjungan</p>
                ...
                @if(in_array($userRole, ['super_admin', 'admin_registrasi']))
                <div class="pt-3 border-t border-slate-800 space-y-1">
                    <p class="px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 mt-3">Data Master</p>
                ...
                {{-- HUMAS & INFORMASI PUBLIK --}}
                @if(in_array($userRole, ['super_admin', 'admin_humas']))
                <div class="pt-3 border-t border-slate-800 space-y-1">
                    <p class="px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 mt-3">Humas & Info Publik</p>
                    ...
                    <a href="{{ route('admin.galeri.index') }}"
                       class="flex items-center px-4 py-3 {{ request()->routeIs('admin.galeri.*') ? 'bg-blue-600 text-white shadow-lg shadow-blue-900/50' : 'text-slate-400 hover:bg-slate-800 hover:text-white' }} rounded-xl transition-all duration-200 group">
                        <i class="fas fa-camera w-5 h-5 mr-3 {{ request()->routeIs('admin.galeri.*') ? 'text-white' : 'text-slate-500 group-hover:text-white' }}"></i>
                ...
                {{-- PENGATURAN SISTEM --}}
                <div class="pt-3 border-t border-slate-800 space-y-1">
                    <p class="px-4 text-[10px] font-black text-slate-500 uppercase tracking-[0.2em] mb-3 mt-3">Pengaturan Sistem</p>
